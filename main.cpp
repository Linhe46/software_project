...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        try {
            ...
            auto value = parser.parse();
            auto result = env->eval(std::move(value));
            if (!read_from_file) std::cout << result->toString() << std::endl;
        } catch (SyntaxError& e) {
            std::cerr << "\033[31m" << "SyntaxError: " << e.what() << "\033[0m"
                      << std::endl;
        } catch (LispError& e) {
            std::cerr << "\033[31m" << "LispError: " << e.what() << "\033[0m"
                      << std::endl;
