...
private:
    ...
    EvalEnv();  // 创建全局环境
    EvalEnv(const std::shared_ptr<EvalEnv>& parent,
            const std::unordered_map<std::string, ValuePtr>&
                inner_params);  // 由参数表创建环境
public:
    ...
    EvalEnvPtr createChild(const std::vector<std::string>& params,
