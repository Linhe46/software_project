...
ValuePtr arithmetic(const std::vector<ValuePtr>& params, arithmeticType func,
    ...
    if (params.size() < least_params)
        throw LispError("At least " + std::to_string(least_params) +
                        " param required");
    else {
        double res = init;
...
ValuePtr numericCompare(const std::vector<ValuePtr>& params,
    ...
    if (params.size() == 0) throw LispError("At least one param required");
...
ValuePtr typeCheck(const std::vector<ValuePtr>& params, typeCheckType func,
...
ValuePtr divi(const std::vector<ValuePtr>& params,
    ...
    return arithmetic(
        params,
        [](double a, double b) {
            if (b == 0) throw LispError("Divided by Zero!");
            return a / b;
        },
        1, 1);
}
...
ValuePtr expt(const std::vector<ValuePtr>& params, EvalEnv& env) {
    if (params.size() != 2)
        ...
        if (x <= 0) throw LispError("expt: a positive base is expected");
...
ValuePtr quotient(const std::vector<ValuePtr>& params, EvalEnv& env) {
    if (params.size() != 2)
        ...
        double x = params[0]->asNumber(), y = params[1]->asNumber();
        if (y == 0) throw LispError("quotient: Divided by zero");
        double res = x / y;
        return std::make_shared<NumericValue>(res > 0 ? std::floor(res)
                                                      : std::ceil(res));
    }
}
ValuePtr modulo(const std::vector<ValuePtr>& params, EvalEnv& env) {
    if (params.size() != 2)
        ...
        if (y == 0) throw LispError("modulo: Cannot apply a zero modulo");
        while (x * y < 0) x += y;
        while (std::abs(x) >= std::abs(y)) x -= y;
...
ValuePtr remainder_(const std::vector<ValuePtr>& params, EvalEnv& env) {
    if (params.size() != 2)
        throw LispError(
            "remainder: Arguments mismatch:\n expected:2\n given: " +
            std::to_string(params.size()));
    else if (!params[0]->isNumber() || !params[1]->isNumber())
        throw LispError("Cannot calculate non-numeric values");
    else {
        double x = params[0]->asNumber(), y = params[1]->asNumber();
        if (y == 0) throw LispError("remainder: Cannot apply a zero remainder");
        return std::make_shared<NumericValue>(
            x - y * (x / y > 0 ? std::floor(x / y) : std::ceil(x / y)));
    }
}
...
ValuePtr eq(const std::vector<ValuePtr>& params, EvalEnv& env) {
    if (params.size() != 2)
        ...
        if (arg1->isSymbol() && arg2->isSymbol())
            return std::make_shared<BooleanValue>(arg1->asSymbol() ==
                                                  arg2->asSymbol());
        else if (arg1->isNumber() && arg2->isNumber())
            return std::make_shared<BooleanValue>(arg1->asNumber() ==
                                                  arg2->asNumber());
        else if (arg1->isBoolean() && arg2->isBoolean())
            return std::make_shared<BooleanValue>(arg1->toString() ==
                                                  arg2->toString());
        else if (arg1->isNil() && arg2->isNil())
            return std::make_shared<BooleanValue>(true);
        else if (arg1->isPair() && arg2->isPair() ||
                 arg1->isProc() && arg2->isProc() ||
                 arg1->isString() && arg2->isString() ||
                 arg1->isLambda() && arg2->isLambda())
...
ValuePtr negation(const std::vector<ValuePtr>& params, EvalEnv& env) {
    if (params.size() != 1)
        throw LispError("not: Arguments mismatch:\n expected:1\n given: " +
                        std::to_string(params.size()));
    auto arg = params[0];
    return std::make_shared<BooleanValue>(arg->toString() == "#f" ? true
                                                                  : false);
}
        ...
        if (arg->isNumber()) {
            ...
            if (std::floor(x) != x)
                throw LispError(
                    ...
                    arg->toString());
            return std::make_shared<BooleanValue>((int)x % 2 == 0 ? true
                                                                  : false);
        } else
            throw LispError(
                "even?: Contract violation\n expected: integer\n given: " +
    ...
    if (params.size() != 1)
        ...
        if (arg->isNumber()) {
            ...
            return std::make_shared<BooleanValue>(
                (int)x % 2 == 0 ? false : true);  // 负数存在，注意不能用x%2==1
        } else
            throw LispError(
                "odd?: Contract violation\n expected: integer\n given: " +
                arg->toString());
...
ValuePtr is_atom(const std::vector<ValuePtr>& params, EvalEnv& env) {
    return typeCheck(
        params,
        [](ValuePtr arg) {
            return arg->isSelfEvaluating() || arg->isSymbol() || arg->isNil();
        },
        "atom?");
}
ValuePtr is_boolean(const std::vector<ValuePtr>& params, EvalEnv& env) {
    return typeCheck(
        params, [](ValuePtr arg) { return arg->isBoolean(); }, "boolean?");
}
...
ValuePtr exit(const std::vector<ValuePtr>& params, EvalEnv& env) {
    if (params.size() == 0 || params.size() == 1) {
        ...
        if (params.size() == 1) {
            try {
                if (is_integer(params, env)) code = params[0]->asNumber();
            } catch (std::runtime_error& e) {
                throw LispError(
                    "exit: Contract violation:\n expected: (exit) (exit "
                    "<val>)");
            }
        }
        std::exit(code);
...
ValuePtr print(const std::vector<ValuePtr>& params, EvalEnv& env) {
    if (params.size() == 0)
        throw LispError(
            "newline: Arguments mismatch:\n expected:at least 1\n given: 0");
    for (auto& i : params) std::cout << i->toString() << ' ';
    std::cout << "\n";
    return std::make_shared<NilValue>();
}
...
ValuePtr map(const std::vector<ValuePtr>& params, EvalEnv& env) {
    ...
    std::transform(elems.begin(), elems.end(), std::back_inserter(new_elems),
                   [&](ValuePtr e) { return EvalEnv::apply(proc, {e}, env); });
...
ValuePtr filter(const std::vector<ValuePtr>& params, EvalEnv& env) {
    ...
    try {
        std::copy_if(elems.begin(), elems.end(), std::back_inserter(new_elems),
                     [&](ValuePtr e) {
                         auto res = EvalEnv::apply(proc, {e}, env);
                         return res->isBoolean() && res->toString() == "#f"
                                    ? false
                                    : true;
                     });
    } catch (LispError& e) {
        throw LispError(
            "filter: Contract violation:\n expected: <proc> should accept one "
            "parameter\n given: " +
            proc->toString());
    }
    if (new_elems.size() == 0) return std::make_shared<NilValue>();
    // return
    // std::make_shared<PairValue>(std::make_shared<SymbolValue>("quasiquote"),std::make_shared<PairValue>(new_elems));
    return std::make_shared<PairValue>(new_elems);
}
            ...
            try {
                return std::accumulate(elems.begin() + 1, elems.end(), elems[0],
                                       [&](ValuePtr e1, ValuePtr e2) {
                                           auto res = EvalEnv::apply(
                                               proc, {e1, e2}, env);
                                           return res;
                                       });
            } catch (LispError& e) {
                throw LispError(
                    "reduce: Contract violation:\n expected: <proc> should "
                    "accept two parameter and be numeric\n given: " +
                    proc->toString());
            }
        }
    }
