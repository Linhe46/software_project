...
EvalEnv::EvalEnv(const std::shared_ptr<EvalEnv>& parent,
                 const std::unordered_map<std::string, ValuePtr>& inner_params)
...
ValuePtr EvalEnv::apply(ValuePtr proc, std::vector<ValuePtr> args,
    ...
    if (proc->isLambda()) {  // lambda表达式
        ...
        throw LispError(
            "application: not a procedure:\n expect a procedure that can be "
    ...
    if (expr->isPair()) {
        ...
        if (expr->isList()) {
            ...
            if (auto name = expr->getCar()->asSymbol())
                if (SPECIAL_FORMS.find(name.value()) !=
                    SPECIAL_FORMS.end())  // 特殊形式
                    return SPECIAL_FORMS.at(name.value())(
                        expr->getCdr()->toVector(),
                        *this);  // at()返回特殊过程的函数指针
            // 过程式
            ValuePtr proc = this->eval(expr->getCar());  // 求解过程
            ValuePtr right = expr->getCdr();
            std::vector<ValuePtr> args =
                evalList(std::move(right));  // 剩余的变量作为过程的参数
            return EvalEnv::apply(proc, args, *this);
        } else
            // return expr;
...
EvalEnvPtr EvalEnv::createChild(const std::vector<std::string>& params,
    ...
    for (int i = 0; i < params.size(); i++) list.insert({params[i], args[i]});
...
ValuePtr LambdaValue::apply(const std::vector<ValuePtr>& args) {
    ...
    ValuePtr res = nullptr;
    for (const auto& proc : body) res = cur_env->eval(proc);
    return res;
}
